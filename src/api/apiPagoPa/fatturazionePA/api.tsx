...
export const getDettaglioFatturaEmessaPa = async (token:string , nonce:string , idEnte:string, idFattura:number) =>{
    const response = await axios.post(`${url}/api/fatture/emesse/dettaglio?nonce=${nonce}`,
        {
            "idEnte": idEnte,
            "idFattura": idFattura
        },
...
export const getDocumentiEmessiPdfPa = async ( token:string ,nonce:string , idFattura:number,idEnte:string) => {
    const response =  await axios.get(`${url}/api/fatture/emesse/dettaglio/download/${idFattura}?idEnte=${idEnte}&nonce=${nonce}`,  
        { headers: {
            Authorization: 'Bearer ' + token
        },}
    );
