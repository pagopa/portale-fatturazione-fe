...
import { TableHead, TableRow, TableCell, Typography, Table,  Tooltip, IconButton, TextField, Popover, TableBody} from "@mui/material";
                  ...
                  <Box  sx={{ margin: 2 , backgroundColor:'#F8F8F8', padding:'10px'}}>
                    <Box style={{overflowY: "auto",maxHeight: "500px", margin:2, backgroundColor:'#F8F8F8'}}>
                      <Table stickyHeader  >
                        ...
                        {loadingCustomAction ? 
                          <SkeletonGridLoading columnLength={5} rowLength={6}/>
                          : detailsSingleRow.map((obj,i)=>{
                            return ( 
                              <TableBody key={`${i}-${obj.ragioneSociale}`}>
                                <TableRow >
                                  <TableCell align="center" sx={{ color:'#0D6EFD', fontWeight:'bold'}}>
                                    <Tooltip title={obj.ragioneSociale}>
                                      <span>
                                        {obj.ragioneSociale?.toString().length > 25
                                          ? obj.ragioneSociale?.toString().slice(0, 20) + '...'
                                          : obj.ragioneSociale}
                                      </span>
                                    </Tooltip>
                                  </TableCell>
                                  ...
                                  <TableCell align="center">{obj.importo}</TableCell>
                                </TableRow>
                              </TableBody>
                            );
                          })}
