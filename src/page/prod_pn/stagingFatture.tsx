...
export interface StagingFattureData {
    count: number
    whitelist: StagingFatture[]
}
export interface StagingFatture {
    idWhite?: number;
...
const StagingFatturePage: React.FC = () => {
  ...
  const [gridData, setGridData] = useState<StagingFatture[]>([]);
  ...
  const {
    ...
    resetFilters,
    isInitialRender
  } = useSavedFilters(PathPf.STAGING_FATTURE, {});
  ...
  const onButtonFiltra = () => {
    ...
    updateFilters({
      body: bodyGetLista,
      pathPage: PathPf.STAGING_FATTURE,
      textValue,
      valueAutocomplete,
      valueSelectMonths,
      page: 0,
  ...
  const onDownload = async () => {
    ...
    await downloadWhiteListPagopa(token, profilo.nonce, bodyGetLista).then((response) => {
      ...
      let fileName = `Fatture in staging/${bodyGetLista.anno}.xlsx`;
      if (bodyGetLista.idEnti.length === 1) {
        fileName = `Fatture in staging/${dataSelect[0].descrizione}/${bodyGetLista.anno}.xlsx`;
      }
      if (bodyGetLista.idEnti.length === 1 && bodyGetLista.mesi.length === 1) {
        fileName = `Fatture in staging/${dataSelect[0].descrizione}/${month[bodyGetLista?.mesi[0] - 1]}/${bodyGetLista.anno}.xlsx`;
      }
  ...
  const buttonsTopHeader = [
    {
      stringIcon: 'Ripristina',
      icon: <RestorePageIcon sx={{ color: selected.length > 0 ? '#1976D2' : '#A2ADB8', cursor: 'pointer' }} />,
      action: 'Restore',
    },
    {
      stringIcon: 'Elimina',
  ...
  return (
    <MainBoxStyled title={'Fatture in staging'}>
...
export default StagingFatturePage;
