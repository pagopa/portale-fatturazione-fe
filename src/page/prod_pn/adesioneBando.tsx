...
const AdesioneBando : React.FC = () => {
    ...
    const columns: GridColDef[] = [
        { field: 'ragioneSociale', headerName: 'Ragione Sociale', width: 200 , headerClassName: 'super-app-theme--header', headerAlign: 'left',  renderCell: (param:{row:Asseverazione}) => <Tooltip title={param.row?.ragioneSociale?.length > 20 ? param.row?.ragioneSociale : null }><span><a className="mese_alidita text-primary fw-bolder" href="/">{param.row?.ragioneSociale?.toString().length > 20 ? param.row?.ragioneSociale?.toString().slice(0, 20) + '...' : param.row?.ragioneSociale}</a></span></Tooltip>},
