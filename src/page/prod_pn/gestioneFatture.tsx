...
export interface GestioneFattureData {
  count: number
  whitelist: GestioneFatture[]
}
...
const GestioneFatture : React.FC = () => {
  ...
  const [gridData, setGridData] = useState<GestioneFatture[]>([]);
  ...
  const { 
    ...
    resetFilters,
    isInitialRender
  } = useSavedFilters(PathPf.GESTIONE_FATTURE,{});
  ...
  const getAnni = async( ) => {
    ...
    await getAnniGestioneFatture(token, profilo.nonce).then(async(res)=>{
  ...
  const getMesi = async(y) => {
    setGetListaLoading(true);
    await getMesiGestioneFatture(token, profilo.nonce, {anno:y}).then((res)=>{
  ...
  const getListTipologiaFattura = async(body) => {
    await getTipologiaFatturaGestioneFatture(token, profilo.nonce,body).then((res)=>{
  ...
  */
  ...
  const onButtonFiltra = () => {
    ...
    updateFilters(
      {
        body:bodyGetLista,
        pathPage:PathPf.GESTIONE_FATTURE,
        textValue,
        valueAutocomplete,
        valueSelectMonths,
        page:0,
  ...
  const onDownload = async() => {
    setShowLoading(true);
    await downloadGestioneFatturePagopa(token, profilo.nonce,bodyGetLista).then((response) =>{
      if (response.ok) {
    ...
    <MainBoxStyled title={"Gestione Fatture"}>
      <ResponsiveGridContainer >
        <MainFilter 
          ...
          keyValue={"anno"}
          keyBody={"anno"}
          arrayValues={arrayYears.map(el => el.toString())}
          extraCodeOnChange={(e)=>{
            if(e.toString() === "9999"){ 
              setBodyGetLista((prev)=> ({...prev, ...{ 
                anno: null,
                mesi: [],
                tipologiaFattura:null
              }}));
              ...
              getMesi(e);
              getListTipologiaFattura({anno:e,mesi:bodyGetLista.mesi});
              setBodyGetLista((prev)=> ({...prev, ...{
                anno: Number(e),
                mesi: [],
                tipologiaFattura:null
        ...
        <MainFilter 
          ...
          extraCodeOnChangeArray={(value)=>{
            ...
            getListTipologiaFattura({anno:bodyGetLista.anno,mesi:valueArray});
            setBodyGetLista((prev) => ({...prev,...{mesi:valueArray,tipologiaFattura:null}}));
          }}
        ...
        <MainFilter 
          ...
          extraCodeOnChange={(e)=>{
            const val =  e.toString() === "9999" ? null : e.toString();
