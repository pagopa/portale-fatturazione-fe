...
export const headerNameListaModuliCommessaSEND: GridColDef[] = [
  { field: 'regioneSociale', headerName: 'Ragione Sociale', width: 200 , headerClassName: 'super-app-theme--header', headerAlign: 'left', align:"left", renderCell: (param:{row:GridElementListaCommesse}) => <Tooltip key={param.row.idEnte} title={param.row.ragioneSociale.length > 20 ? param.row.ragioneSociale : null }><span><a className="mese_alidita text-primary fw-bolder" href="/">{param.row.ragioneSociale?.toString().length > 20 ? param.row.ragioneSociale?.toString().slice(0, 20) + '...' : param.row.ragioneSociale}</a></span></Tooltip>},
  ...
  { field: 'dataContratto', headerName: 'Data Contratto', width: 150, headerClassName: 'super-app-theme--header', headerAlign: 'center', align:"center", valueFormatter: ({ value }) => value?.split('T')[0]||"--"},
  ...
  { field: 'dataChiusura', headerName: 'Data Chiusura', width: 150, headerClassName: 'super-app-theme--header', headerAlign: 'center', align:"center", renderCell: ((param:{row:GridElementListaCommesse}) =>(param.row.source === "archiviato" ? "--" : param.row.source === "facoltativo" ? "TBD" : (param.row.dataChiusura?.split('T')[0]|| "--")))},
